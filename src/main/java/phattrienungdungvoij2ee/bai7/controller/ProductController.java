...
public class ProductController {
    ...
    public String listProducts(Model model,
        ...
        Page<Product> productPage = productService.getAllProductsWithFilterAndSort(page - 1, pageSize, sort,
                categoryId);
    ...
    public String search(Model model, @RequestParam String key) {
        // 1. Lấy danh sách kết quả (Kiểu List thông thường)
        List<Product> listProduct = productService.GetSearchProducts(key);

        // 2. Bọc List thành đối tượng Page để file HTML không bị lỗi phân trang
        // Nếu danh sách rỗng thì gán size = 1 để tránh lỗi chia cho 0
        int pageSize = listProduct.isEmpty() ? 1 : listProduct.size();
        Page<Product> productPage = new PageImpl<>(listProduct, PageRequest.of(0, pageSize), listProduct.size());

        // 3. Gửi toàn bộ dữ liệu cần thiết sang View
        model.addAttribute("products", productPage);
        model.addAttribute("currentPage", 1); // Tìm kiếm xong mặc định đứng ở trang 1
        model.addAttribute("categories", categoryService.getAllCategories()); // Tránh lỗi 500 ở cái Dropdown
        model.addAttribute("key", key); // Giữ lại từ khóa trên thanh tìm kiếm
    ...
    public String showAddForm(Model model, HttpSession session) {
        ...
        // Kiểm tra: Chưa đăng nhập HOẶC không phải ADMIN thì đuổi về
